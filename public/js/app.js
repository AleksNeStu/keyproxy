        ...
        async function loadConfiguration() {
            try {
                ...
                renderConfigEnvFiles();

                // Update loaded files indicator
                const enabledCount = data.enabledCount || configEnvFiles.filter(f => !f.disabled).length;
                document.getElementById('configActiveFile').textContent =
                    configEnvFiles.length === 0
                        ? 'No key files configured'
                        : `${enabledCount} of ${configEnvFiles.length} file(s) enabled`;
        ...
        function renderConfigEnvFiles() {
            const list = document.getElementById('configEnvFileList');

            if (!configEnvFiles || configEnvFiles.length === 0) {
                list.innerHTML = '<div class="text-center text-muted-foreground text-sm py-4">No rotation key files configured. Add a dotenv file containing API keys.</div>';
                return;
            }

            list.innerHTML = configEnvFiles.map((file, index) => {
                const priorityBadge = getPriorityBadge(file.priority);
                const disabledClass = file.disabled ? 'opacity-50' : '';

                return `
                    <div class="config-env-item flex items-center gap-2 p-2 rounded bg-card border border-border hover:border-border transition-colors ${disabledClass}"
                         data-env-name="${escapeHtml(file.name)}" data-priority="${file.priority}">
                        <!-- Drag Handle -->
                        <div class="cursor-grab active:cursor-grabbing p-1 text-muted-foreground hover:text-foreground" title="Drag to reorder">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                <span class="text-sm font-medium text-foreground truncate">${escapeHtml(file.name)}</span>
                                ${file.disabled ? '<span class="px-1.5 py-0.5 text-xs bg-muted text-muted-foreground rounded-full">Disabled</span>' : '<span class="px-1.5 py-0.5 text-xs bg-green-500/20 text-green-400 rounded-full">Loaded</span>'}
                            </div>
                            <div class="text-xs text-muted-foreground truncate" title="${file.path}">${file.path}</div>
                        </div>

                        <!-- Up/Down Buttons -->
                        <div class="flex items-center gap-1">
                            <button onclick="moveEnvFilePriority(${index}, -1)" class="p-1 hover:bg-muted rounded transition-colors ${index === 0 ? 'opacity-30 cursor-not-allowed' : ''}" ${index === 0 ? 'disabled' : ''} title="Move up">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                                </svg>
                            </button>
                            <button onclick="moveEnvFilePriority(${index}, 1)" class="p-1 hover:bg-muted rounded transition-colors ${index === configEnvFiles.length - 1 ? 'opacity-30 cursor-not-allowed' : ''}" ${index === configEnvFiles.length - 1 ? 'disabled' : ''} title="Move down">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button onclick="toggleConfigEnvFileDisabled('${file.name}')" class="p-1.5 hover:bg-muted rounded transition-colors ${file.disabled ? 'text-muted-foreground' : 'text-green-500'}" title="${file.disabled ? 'Enable file' : 'Disable file'}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ${file.disabled
                        ...
                        <!-- Remove Button -->
                        <button onclick="removeConfigEnvFile('${file.name}')" class="p-1.5 hover:bg-red-500/10 text-red-500 rounded transition-colors" title="Remove file">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        function getPriorityBadge(priority) {
            const colors = {
                1: 'bg-gradient-to-r from-yellow-500 to-amber-600 text-yellow-900 shadow-lg shadow-yellow-500/30',
                2: 'bg-gradient-to-r from-gray-300 to-gray-400 text-gray-800 shadow-lg shadow-gray-400/30',
                3: 'bg-gradient-to-r from-orange-600 to-amber-700 text-orange-50 shadow-lg shadow-orange-600/30'
            };
            const defaultColor = 'bg-muted text-muted-foreground';

            const colorClass = colors[priority] || defaultColor;

            return `
                <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${colorClass}" title="Priority ${priority}">
                    ${priority}
                </div>
            `;
        }

        async function moveEnvFilePriority(index, direction) {
            const newIndex = index + direction;
            if (newIndex < 0 || newIndex >= configEnvFiles.length) return;

            // Swap items
            [configEnvFiles[index], configEnvFiles[newIndex]] = [configEnvFiles[newIndex], configEnvFiles[index]];

            // Update priority numbers
            configEnvFiles.forEach((file, i) => file.priority = i + 1);

            // Get the new order
            const newOrder = configEnvFiles.map(f => f.name);
        ...
        async function toggleConfigEnvFileDisabled(name) {
            try {
                const res = await fetch('/admin/api/toggle-env-file-disabled', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name })
                });
                const data = await res.json();
                if (data.success) {
                    const file = configEnvFiles.find(f => f.name === name);
                    const newState = !file.disabled;
                    showSuccessToast(`${name} ${newState ? 'disabled' : 'enabled'}`);
                    loadConfiguration();
                } else {
                    showErrorToast('Failed to toggle state');
                }
            } catch (err) {
                showErrorToast('Failed to toggle state: ' + err.message);
            }
        }

        async function addConfigEnvFile() {
            const name = document.getElementById('configNewEnvName').value.trim();
            const filePath = document.getElementById('configNewEnvPath').value.trim();

            if (!name || !filePath) {
                showErrorToast('Please enter both name and path');
                return;
            }
            ...
            try {
                ...
                if (res.ok) {
                    document.getElementById('configNewEnvName').value = '';
                    document.getElementById('configNewEnvPath').value = '';
        ...
        async function removeConfigEnvFile(name) {
